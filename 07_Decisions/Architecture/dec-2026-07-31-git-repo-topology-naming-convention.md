...
개정문의 결정을 저장소 6개에 적용했다. 규약 도입 전후를 같은 방법으로 쟀다.

| | 도입 전 | 적용 후 |
|---|---|---|
| 코드 층 위반 (우리가 소유한 저장소) | **313건** | **0건** |
| 그 중 규칙 자신의 오탐 | 292건 | — |
| 실제 개명 | — | 디렉토리 7 · 배포명 10 |
| 남은 유예 (만료 있음) | — | 2건 |

**313 중 292 가 규칙의 오탐이었다.** 실측하지 않고 규약만 썼다면 그 292 가 그대로 강제됐을 것이고, 정당한 이름이 무더기로 걸리는 규칙은 반드시 우회된다. 넷을 실측으로 정정했다 — 모듈 상수 케이스(188건 오탐), 레이아웃 변형의 강제 범위(infra 의 층을 침범했다), 볼트의 번호 접두 폴더, 저장소 고유 디렉토리.

탐색력도 쟀다. 규약에서 유도한 glob 이 실제로 맞히는가 — **9개 질문 전부 적중, 잡음 0, 질문당 패턴 1개.** 이 측정은 일회성이 아니라 상시 게이트로 남아, 규약을 고쳤는데 glob 이 더 이상 맞히지 않으면 CI 가 실패한다.

### 다음 회고에 대한 교훈 — 그리고 그것을 검사로 바꿨다

회고 일정도 검사를 가져야 한다. 지금은 사람이 날짜를 기억해야 하고, 이번에 20일 실패했다.

**`/vault-audit` 에 STEP 10 을 더했다.** `retrospective_date` 가 지났는데 `## Retrospective` 섹션이 없으면 warning, 30일을 넘으면 critical 이다. 30일을 critical 로 올린 근거는 이번 실측이다 — 20일에 네 곳이 갈렸다.

D4 가 *"사람 규율이 아니라 도구"* 라고 못박았는데 그것이 회고 일정 자신에게는 적용되지 않았던 것이 이 회고가 늦은 이유이고, 이제 그 빈칸이 닫혔다.
## 관련 자료
