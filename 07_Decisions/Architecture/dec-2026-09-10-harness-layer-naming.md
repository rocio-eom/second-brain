...
- **후속 3 (`platform-request` 개명) 무효.** 유예 만료를 2026-10-01 로 걸어 뒀으나
  **그 이름은 이 ADR 문서 안에서만 존재한다** — 워크스페이스 여섯 저장소 어디에도 그런 자산이 없다
  (2026-09-14 실측). 대상 없는 유예는 만료일에 없는 것을 찾게 만들므로 여기서 닫는다.
  이 ADR 의 실측 표가 전제한 세계가 재구축되지 않았다는 증거가 하나 더 늘었다.
- **후속 4 (재검토 트리거 6개) 이관.** ADR 0014 가 없으므로 트리거는
  ...
  이관 과정에서 트리거 하나(`scan` 2회 연속 스킵)가 **구조적으로 발화 불가**임이 드러나
  측정 가능한 것으로 대체했다 — 켠 저장소는 자산 ≥1건이라 스킵되지 않고, 켜지 않은 저장소는
  워크플로 자체가 없어 `scan` 이 돌지 않는다.
