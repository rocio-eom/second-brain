...
modified: 2026-06-02
status: draft
tags: [aws, aws-s3-vectors, kms, encryption, multitenancy, security]
domain:
  - ai-ml-llm
aliases: [S3 Vectors encryption, SSE-KMS for S3 Vectors, per-index CMK, S3 Vectors multitenancy]
literature_source: []
related:
...
# Amazon S3 Vectors — 인덱스별 KMS CMK 멀티테넌트 패턴
...
- **설정 레벨**: 버킷 default / **인덱스 override** (둘 중 인덱스가 우선).
- **키 식별**: **전체 ARN 강제** — `arn:aws:kms:ap-northeast-2:<account>:key/<key-id>`. Alias 미지원.
- **권한**: KMS 키 정책에 `s3vectors.amazonaws.com` 서비스 프린시플 + 사용 IAM 역할에 `kms:GenerateDataKey`, `kms:Decrypt`.
...
graph TD
  ...
  KEY_POL[KMS Key Policy<br/>s3vectors.amazonaws.com<br/>+ IAM role principal] -.allow.-> CMK_T1
...
KB 생성 시 vector store CMK를 지정하면 KB가 자동으로 vector index에 동일 CMK를 설정. KB 서비스 롤이 CMK 정책에 추가돼야 함. 미설정 시 ingestion job이 KMS AccessDeniedException로 실패.

### 단일 버킷 / 다중 CMK 패턴
한 vector bucket 안에서 인덱스별 CMK만 다르게 두는 구성. **버킷 정책·CFN 스택은 공유**하면서 키 단위 격리·감사 가능. 운영 단순성과 격리의 균형.

## 활용 시나리오

### 시나리오 1: 멀티테넌트 RAG SaaS
테넌트당 인덱스 1개 + 인덱스별 CMK. 테넌트 onboarding = (1) CMK 생성 → (2) 키 정책에 S3 Vectors 서비스 프린시플 + 테넌트 전용 IAM 역할 추가 → (3) `CreateVectorIndex` with encryption override → (4) tenant ID를 filterable metadata에 부착.

### 시나리오 2: BYOK / 키 소유 분리
일부 엔터프라이즈 고객은 자기 AWS 계정의 KMS 키를 사용 요구. SaaS 계정의 vector bucket이 cross-account CMK를 사용하도록 grant 부여 — 키 소유는 고객, 데이터 처리는 SaaS.

### 시나리오 3: 컴플라이언스 키 회전·삭제
연 1회 자동 키 회전 활성화. 인덱스의 기존 데이터는 이전 키 자료(key material)로 복호화 가능, 신규 적재는 새 키 자료로. 테넌트 이탈 시 CMK schedule deletion → 해당 인덱스 데이터 사실상 **crypto-shredding**.

## 장단점 및 트레이드오프

| 항목 | 내용 |
|---|---|
| 장점 | (1) **인덱스 단위 격리**가 멀티테넌트 SaaS에 최적 (2) BYOK·crypto-shredding·키 정책 자유로 컴플라이언스 친화 (3) CloudTrail 키별 감사로 누가 무엇을 복호화했는지 명확 (4) 버킷 default와 인덱스 override의 2단 구성이 운영 단순 |
| 단점 | (1) KMS 비용(요청·키 보관)이 PUT/Query마다 누적 — 고QPS에서 의미 있음 (2) **full ARN 강제** — 키 별칭/ID 사용 자동화 코드는 재작성 필요 (3) 키 정책에 서비스 프린시플·IAM 역할 누락 시 모든 작업이 KMSAccessDenied로 실패 (4) cross-account CMK 사용 시 정책·grant 복잡도 ↑ |
| 트레이드오프 | "격리·컴플라이언스" vs "비용·운영 복잡도". 모든 인덱스에 CMK를 두면 비용·운영이 부담 — **민감 인덱스만 CMK, 나머지는 SSE-S3** 권장. |

## 함정 및 안티패턴

- **KMS 키 alias / ID로 설정 시도**: full ARN 강제 — CLI/SDK가 잘못된 식별자로 호출하면 InvalidArgumentException. CFN 템플릿에서 `!Sub` / `!GetAtt`로 ARN을 동적 생성.
- **CMK 정책에 서비스 프린시플 누락**: `s3vectors.amazonaws.com`이 `kms:GenerateDataKey`, `kms:Decrypt` 권한이 없으면 PutVectors가 **즉시 실패**. 키 정책 템플릿이 첫 트러블슈팅 포인트.
- **모든 인덱스에 CMK 적용**: KMS 비용 + 운영 부담 누적. 격리가 필요한 워크로드만 CMK, 나머지는 SSE-S3로 비용·복잡도 절약.
- **CMK 회전 = 데이터 재암호화 가정**: KMS는 키 자료를 회전하지만 **기존 데이터는 이전 키 자료로 복호화**됨. 명시적 재암호화는 별도 작업(재인제스트).
- **테넌트 삭제 후 CMK만 disable**: KMS는 30~365일 대기 후 삭제. 그 기간 동안 인덱스 데이터는 복구 가능 상태. **즉시 무결성 보장**이 필요하면 인덱스 삭제 + CMK schedule deletion 병행.
