...
modified: 2026-06-02
status: draft
tags: [vector-search, embedding, versioning, migration, concept, lock-in]
domain:
  - ai-ml-llm
aliases: [Embedding Model Lock-in, Vector Index Lock-in, Re-embedding strategy]
literature_source: []
related:
...
# Embedding Model Lock-in — 벡터 인덱스의 모델 잠금

## 핵심 요약

서로 다른 임베딩 모델은 **서로 다른 의미 공간**에 벡터를 만든다. Titan v2의 1024차원과 Cohere v3의 1024차원은 차원 수가 같아도 **호환되지 않음** — cosine / dot product가 무의미하다. S3 Vectors는 (1) 인덱스 생성 시 차원·메트릭을 **불변**으로 고정하고, (2) 어떤 모델로 임베딩했는지 메타에 직접 강제하지 않기 때문에, **사실상 인덱스 단위로 모델이 잠긴다**. 모델 교체는 **새 인덱스 + 전 코퍼스 재임베딩**이 정석이고, 그 비용·다운타임이 운영의 핵심 위험.
...
S3 Vectors 인덱스는 생성 시 $(d, D)$가 고정되므로:
...
- **모델 = 언어**: Titan은 한국어, Cohere는 일본어. 같은 알파벳 수(차원) 가져도 단어가 다름.
...
## 유사 기술 비교 (모델 교체 비용 측면)

| 모델 변경 시나리오 | 비용 / 절차 |
|---|---|
| 동일 모델 minor 버전 | 보통 호환 (모델 카드 확인 후 무재임베딩 가능) — but 검증 필수 |
...
| Re-embedding | Re-indexing (인덱스 알고리즘 변경) | 전자는 입력 임베딩 자체, 후자는 ANN 그래프/IVF 재구성 |
| Dimension match | Semantic compatibility | 차원 같다고 의미 공간 같지 않음 |
| Drift | Embedding drift | Drift는 데이터 분포 변화, embedding drift는 모델 업그레이드로 인한 좌표계 변화 |
...
새 쿼리를 옛 임베딩 공간으로 매핑하는 학습 가능 어댑터. 전 코퍼스 재임베딩을 미루는 **near-zero downtime** 업그레이드 전략. 단, recall에 약간의 손실.
...
구·신 인덱스 병행 → 신규 입력은 양쪽에 적재 → 검색은 신 인덱스 우선, miss 시 구 인덱스 fallback → 옛 데이터 점진 재임베딩 → 옛 인덱스 폐기. S3V의 인덱스당 추가 비용 없음이 이 패턴을 저렴하게 만듦.
...
- [I Updated My Embedding Model and My RAG Broke — Decompressed.io](https://decompressed.io/learn/rag-observability-postmortem) — 사후 분석 (High)
- [Different Embedding Models, Different Spaces — Gary Stafford / Medium](https://garystafford.medium.com/different-embedding-models-different-spaces-the-hidden-cost-of-model-upgrades-899db24ad233) — 호환성 분석 (High)
- [Embedding Models in Production: Versioning and Index Drift — TianPan](https://tianpan.co/blog/2026-04-09-embedding-models-production-versioning-index-drift) — 버전 관리 (High)
- [Drift-Adapter — arXiv 2025](https://arxiv.org/pdf/2509.23471) — 어댑터 기반 업그레이드 (High)
- [When Good Models Go Bad — Weaviate](https://weaviate.io/blog/when-good-models-go-bad) — 모델 교체 위험 (Mid)
...
- [langchain Neo4j embedding dim incompatibility — GitHub #16336](https://github.com/langchain-ai/langchain/issues/16336) — 차원 불일치 사례 (Mid)
