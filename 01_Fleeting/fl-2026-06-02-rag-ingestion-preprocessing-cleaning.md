...
domain:
  - ai-ml-llm
aliases: [Preprocessing, Data Cleaning, Deduplication, PII Redaction, Text Normalization, RAG Data Cleaning]
literature_source: []
related:
...
# RAG Ingestion - Preprocessing & Cleaning
...
- **안티패턴 1**: dedup을 chunking 후에 수행 → chunk 경계 차이로 동일 문장이 다른 hash → exact dedup 효과 거의 없음 → **대안**: 문서/문단 단위 dedup을 chunking 전에 수행
