...
modified: 2026-06-02
status: draft
tags: [vector-search, rag, architecture-design, chunking, embedding, retrieval, hybrid-search]
domain:
  - backend
aliases: [Vector Search Architecture Design, Search and Indexing Architecture Design, Vector Indexing Architecture]
literature_source: []
related:
...
# Vector Search Architecture - Search/Indexing Architecture Design

## 핵심 요약

Vector 기반 검색·색인 아키텍처를 **설계**할 때 다루는 핵심 결정: ① 데이터 수집·청킹 전략 ② 임베딩 모델/차원/metric 선택 ③ vector store 선택과 인덱스 토폴로지 ④ 검색 시 query 변환·하이브리드·리랭킹 파이프라인. 운영 단계의 결정(재인덱싱·모니터링·트래픽 변동 대응)은 [[fl-2026-06-02-vector-search-operations]] 참조.
...
자매 노트: OpenSearch 특화 구성은 [[fl-2026-06-02-vector-search-opensearch-knn]], 제품 선택은 [[fl-2026-06-02-vector-search-db-comparison]].
...
4. **Vector store 결정**: 워크로드·기존 스택·운영 모델 매트릭스로 선정 → [[fl-2026-06-02-vector-search-db-comparison]]
...
- **안티패턴 5: 차원·metric 임의 변경** → 인덱스 전체 무효화 → 모델·dim·metric을 인덱스 이름/메타데이터에 박제(자세한 운영 절차는 [[fl-2026-06-02-vector-search-operations]])
