...
modified: 2026-06-02
status: draft
tags: [aws, aws-bedrock, rag, ai-ml-llm, managed-service, vector-db, knowledge-bases, opensearch, aurora, pgvector, graphrag]
domain:
  - ai-ml-llm
...
- **2024년 GA 이후 빠른 확장**: 2024년 12월 GraphRAG preview → 2025년 3월 GA, 2025년 4월 Aurora/MongoDB hybrid search, 2025년 12월 S3 Vectors 출시(최대 90% 비용 절감)
...
2. **Parse**: Bedrock Data Automation parser는 표·이미지·heading 구조 보존 (페이지당 $0.010)
...
| Reranking              | Cohere Rerank 1.0 통합 — $1.00/1,000 query                                                                                                                                             |
...
| 단점     | "black box" 일부 — chunking·retrieval 세부 튜닝 제약, 기본 쿼터 낮음 (500 TPM 수준) | AWS 외부 환경, vendor lock-in                  | 운영·튜닝 부담, managed 영역 직접 책임            | 구축·운영 비용 최대                      |
| 적합 케이스 | AWS 중심 stack, 빠른 출시, ML 엔지니어 부족                                     | 빠른 출시 + AWS 외부 환경                          | 복잡 retrieval, 다중 vector store 혼용      | retrieval 정확도가 핵심 KPI, 충분한 리소스   |

## 실제 사례

### Ring (Amazon)
글로벌 고객 지원에 사용. 기존 Lex 기반 rule-based 챗봇은 정형 대화 한계로 피크 시간대 16%가 human agent로 escalation. Bedrock Knowledge Bases 도입 후 ① region 별 컨텐츠를 metadata 필터로 분리 ② ingestion/평가/promotion 워크플로우 분리해 컨텐츠 거버넌스 강화 ③ 비용 절감과 함께 글로벌 스케일 달성.

### MetaNet SAS (한국)
MetaPay에 생성형 AI 도입. Bedrock 기반으로 한국 연말정산 같은 도메인 정확도 critical 업무를 더 빠르고 정확하게 처리. 한국 SI 기업의 Bedrock KB 적용 사례 중 공개 사례.

### AWS 전체 어답션
Bedrock은 2026년 기준 100,000+ 조직이 사용. AWS re:Invent 2025에서 multimodal ingestion, S3 Vectors(비용 90%↓), structured data retrieval, enterprise governance 등이 추가됨.
...
**적용**: S3 connector + hierarchical chunking + Titan v2 1024 dim + OpenSearch Serverless → `RetrieveAndGenerate` + Claude 3.5 Sonnet → Guardrails로 PII 필터.

### 시나리오 2: 비용 90% 절감 (S3 Vectors 전환)
**컨텍스트**: OpenSearch Serverless 최소 2 OCU($345/월)가 트래픽 적은 KB에 부담.  
**선택 이유**: S3 Vectors는 trillion-scale 지원, sub-second latency, 최대 90% 절감.  
...
| 단점     | • Chunking·retrieval 내부 세부 튜닝에 제약 — fine-grained scoring 함수, 커스텀 retriever 어려움<br/>• AWS 외부 vendor lock-in (Bedrock 외 LLM 호출 어려움)<br/>• 기본 service quota가 낮음 — 프로덕션 트래픽에는 ticket으로 상향 필요<br/>• OpenSearch Serverless 기본값은 idle 비용 $345/월 (S3 Vectors로 일부 해결됨)                                                                 |
...
- **안티패턴 1: OpenSearch Serverless 기본값으로 모든 KB 생성** → 트래픽 적은 KB도 idle $345/월 발생 → 트래픽·기능 요구를 평가해 S3 Vectors / Aurora pgvector 등 적합 store 선택, 기존 KB는 migration 검토
- **안티패턴 2: 기본 service quota로 production 가정** → 500 TPM 수준 한도에 production traffic이 throttle → 출시 전 quota 상향 ticket 제출 (수동 승인 절차 + 사용 사례 입증 필요)
...
- [Amazon Bedrock Review (2026): Is It Production Ready? (truefoundry)](https://www.truefoundry.com/blog/our-honest-review-of-amazon-bedrock-2026-edition) — quota·비용·한계 비판적 리뷰
...
- [Cutting Bedrock KB Costs by ~90%: OpenSearch Serverless → Aurora pgvector](https://ercanermis.com/cutting-amazon-bedrock-knowledge-base-costs-by-90-migrating-from-opensearch-serverless-to-aurora-serverless-v2-with-pgvector/) — 비용 최적화 실측 사례
