...
promoted_from: fl-2026-06-02-context-aware-chunking-overview
related:
...
Context-aware Chunking은 문서를 단순 토큰/문자 수가 아니라 **의미·구조·문맥 경계**를 인식해 분할하는 RAG 전처리 기법 집합이다. naive fixed-size chunking이 문장 중간을 자르거나 헤더·표를 깨뜨려 검색 품질을 떨어뜨리는 문제를 해결한다. 본 노트는 본 카테고리의 **정의·구성 기법·선택 원칙**을 다루며, 개별 sub-strategy 동작은 sibling 노트들이 담당한다.

- **스펙트럼 위에서 선택**: 가장 저렴한 fixed-size부터 LLM이 직접 분할 위치를 결정하는 agentic chunking까지 비용·품질 trade-off가 존재한다.
- **다층 전략 결합이 표준**: 본 노트 작성 시점(2026-06) 기준 단일 전략보다 structure-aware → semantic → contextual retrieval을 순차 또는 병렬 적용하는 하이브리드가 우세.
...
Context-aware Chunking은 하위 전략(Semantic / Structure-aware / Contextual Retrieval)마다 청크 길이 분포·임베딩 활용 방식이 다르므로, 단일 임베딩 모델을 권장하기보다 **하위 전략별 매트릭스**로 정리한다. 결정 룰·전체 매트릭스는 [[chunking-embedding-compatibility-matrix]] 참조.
...
### Anthropic Claude
2024년 9월 Contextual Retrieval 발표. Claude로 각 chunk에 문서 컨텍스트 prefix를 생성, prompt caching으로 비용을 백만 토큰당 약 $1.02 수준으로 절감(2024-09 공시가 기준). Contextual Embeddings + Contextual BM25 + reranking 조합으로 검색 실패율을 최대 67% 감소시킨 결과를 공개했다.
...
| 항목 | 내용 |
|---|---|
| 장점 | 검색 정확도 개선(실패율 최대 49–67% 감소, Anthropic 공시), 도메인 적응성, 산문·표·코드 혼합 문서 처리 |
| 단점 | 파이프라인 복잡도 증가, contextual augmentation은 LLM 호출 비용 발생, 평가·튜닝 코스트 |
| 트레이드오프 | 품질 vs 비용·지연. 단순 문서에는 over-engineering 위험. 프로토타이핑은 fixed-size로 시작 후 단계적 도입 권장. 측정 방법론은 [[chunking-strategy-methodology]] |
...
- **평가 누락**: 청킹 전략 변경의 검색 품질 영향을 측정하지 않음 → recall@k, MRR 등 지표로 A/B 비교 필수. 실패 유형 진단은 [[chunking-failure-modes]]
...
- [[chunking-strategy-methodology]] — 전략 선택·측정·개선 6단계 방법론 (process 측)
- [[chunking-embedding-compatibility-matrix]] — 전략 × 임베딩 매트릭스 (max_seq·차원·다국어 결정 룰)
- [[chunking-failure-modes]] — Mid-context loss·Context starvation·Boundary noise 진단 루프
- [[semantic-chunking]] — 임베딩 거리 기반 경계 탐지 (본 카테고리의 의미 축 멤버)
- [[document-structured-chunking]] — Markdown/HTML/코드 구조 마커 기반 분할 (구조 축 멤버)
- [[agentic-chunking]] — LLM이 분할점 결정 (스펙트럼 반대 끝)
- [[fixed-size-chunking]] — baseline 비교 대상
- [[recursive-chunking]] — 평문 디폴트 대안
- [[sliding-window-chunking]] — retrieval·합성 단위 분리
