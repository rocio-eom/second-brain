...
domain: ai-ml-llm
moc: "[[moc-chunking]]"
tags: [rag, chunking, contextual-retrieval, anthropic, claude, prompt-caching, bm25, hybrid-search]
aliases: [Contextual Retrieval, Contextual Embeddings, Contextual BM25, Anthropic Contextual Retrieval]
promoted_from: fl-2026-06-02-context-aware-chunking-contextual-retrieval
related:
...
- **Prompt caching이 비용 핵심**: 전체 문서를 캐시해 chunk 수만큼 반복 호출 시 비용을 ~90% 절감
- **Reranking과 결합 시 효과 극대화**: Contextual Retrieval 단독 49% 실패율 감소, reranker 결합 시 67% 감소(Anthropic 공식 측정)

> **버전 민감 항목**: 비용 수치($1.02/백만 토큰)는 Anthropic 가격 정책 변동에 따라 달라질 수 있음. 최신 가격은 [Anthropic pricing](https://www.anthropic.com/pricing) 확인.
...
| Prompt Caching 활용 | 전체 문서를 한 번 캐시, chunk 수만큼 호출 시 캐시 토큰은 90% 할인. `cache_control: ephemeral` 사용 |
| Hybrid Search Fusion | vector 유사도와 BM25 점수를 결합(예: Reciprocal Rank Fusion) |
| Reranking layer | 1차 검색 결과 상위 K개를 cross-encoder로 재순위 |

### 임베딩 모델 호환성

Contextual Retrieval은 청크 앞에 LLM이 생성한 **컨텍스트 prefix 50~100토큰을 추가**한 뒤 임베딩한다. 따라서 임베딩 모델의 max_seq가 확장 청크를 수용해야 하며, BM25와의 hybrid 결합에도 강한 모델이 적합하다.
...
| `bge-m3` | 1024 / 8192 | 다국어 + sparse(BM25) 병행 — Contextual Retrieval의 BM25 결합 흐름과 정합 |
...
| 장점 | 검색 실패율 최대 67% 감소, 키워드+의미 동시 강화 | 의미 경계 보존, 구현 단순 | LLM 추론 비용 없음, cross-reference 보존 |
...
공식 Cookbook에서 prompt caching을 활용한 비용 절감 코드와 함께 ParagraphChunker → Claude 호출 → 임베딩의 end-to-end 파이프라인을 제공한다. 구현 상세는 [Claude Cookbook — Contextual Embeddings Guide](https://platform.claude.com/cookbook/capabilities-contextual-embeddings-guide) 참조.

### LlamaIndex Contextual Retrieval
...
조항 간 cross-reference(예: "제3조에서 정의한 바에 따라")가 많은 계약서에서, 단일 chunk만으로는 "어떤 계약의 어떤 조항인지" 모호하다. 조항 단위 분할 → 각 chunk에 "이 조항은 X 계약 Y조의 일부로, 앞 조항에서 정의한 Z를 참조" 형식 prefix를 Claude로 생성 → Contextual Embeddings + Contextual BM25 + cross-encoder reranker.

### 시나리오 2: 사내 컴플라이언스·감사 자료

부서·연도·규제 종류가 다양한 정책 문서에서 chunk에 "어떤 부서·연도·규정"인지 명시되지 않으면 잘못된 문서가 검색된다. 문서 메타데이터(부서, 연도, 규정 ID)를 Claude prompt에 함께 주어 prefix를 생성, retrieval 시 metadata filter와 결합.

### 시나리오 3: 코드 저장소 + 문서 통합 검색

API reference, tutorial, source code를 한 인덱스에서 검색할 때 함수 코드 chunk만으로는 "어떤 모듈의 어떤 책임"인지 모호하다. 모듈 README와 함수 docstring을 prompt cache에 적재, 각 함수 본문에 대해 "이 함수는 X 모듈의 Y 기능을 담당, 호출자는 Z" prefix를 생성.
...
- **BM25 인덱스 생략**: Contextual Embeddings만 적용하고 BM25를 빼면 키워드 누락 사례 보완 불가 → 두 인덱스 모두 구축.
...
- **max_seq 512 이하 임베딩 모델 사용**: prefix 추가로 chunk 길이가 늘어나 자주 잘림 → 전략 효과 무효화. max_seq 2K 이상 모델 선택.

## 참고 자료

- [Anthropic — Introducing Contextual Retrieval](https://www.anthropic.com/news/contextual-retrieval) — 공식 발표, 메트릭과 방법론 (High)
- [Claude Cookbook — Contextual Embeddings Guide](https://platform.claude.com/cookbook/capabilities-contextual-embeddings-guide) — 공식 구현 가이드와 코드 (High)
- [LlamaIndex Contextual Retrieval Cookbook](https://developers.llamaindex.ai/python/examples/cookbooks/contextual_retrieval/) — LlamaIndex 통합 예제 (High)
- [DataCamp — Anthropic's Contextual Retrieval: A Guide With Implementation](https://www.datacamp.com/tutorial/contextual-retrieval-anthropic) — 단계별 튜토리얼 (Mid)
- [Instructor — Implementing Contextual Retrieval with Async Processing](https://python.useinstructor.com/blog/2024/09/26/implementing-anthropics-contextual-retrieval-with-async-processing/) — 비동기 처리 패턴 (Mid)

## 관련 노트

- [[chunking-strategy-methodology]] — 청킹 전략 선택·측정·개선 방법론 (Contextual Retrieval은 augmentation 레이어로 기존 전략 위에 추가)
- [[chunking-embedding-compatibility-matrix]] — max_seq 기반 청킹 전략 × 임베딩 모델 호환 매트릭스
- [[semantic-chunking]] — Contextual Retrieval과 비교 대상인 동적 경계 청킹 전략
- [[rag-embedding-generation]] — prefix 붙은 augmented chunk의 배치 임베딩 처리량·비용
- [[rag-document-loading]] — Contextual Retrieval 직전 단계: 파싱 품질이 prefix 생성 품질의 상한
- [[recall-vs-filter-tradeoff]] — BM25 hybrid 검색에서의 precision/recall 트레이드오프
