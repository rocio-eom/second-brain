...
domain: ai-ml-llm
moc: "[[moc-chunking]]"
tags: [rag, chunking, document-based, markdown, html, code, structured]
aliases: [Document-based Chunking, Structure-aware Chunking, Markdown Chunking, HTML Chunking, Code Chunking]
promoted_from: fl-2026-06-03-chunking-strategy-document-structured
related:
  - "[[fixed-size-chunking]]"
  - "[[recursive-chunking]]"
  - "[[semantic-chunking]]"
...
- [[recursive-chunking]] — document-structured의 fallback. 구조 청크가 토큰 한도 초과 시 2차 분할로 결합
- [[fixed-size-chunking]] — 평문·OCR 결과 등 구조가 없는 문서에서 document-structured의 대안
- [[semantic-chunking]] — 형식이 일관되지 않은 문서에서 헤딩 대신 의미 거리로 경계 결정
- [[sliding-window-chunking]] — 구조 청크 + sentence-window retrieval 결합으로 인용 정확도 극대화
- [[agentic-chunking]] — LLM이 형식 감지·전략 선택을 모두 자동화. document-structured의 적응형 일반화
