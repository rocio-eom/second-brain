...
promoted_from: fl-2026-06-02-aws-bedrock-embedding
related: []
---
...
| Cohere Embed v3 (English / Multilingual) | 텍스트 전용, 다국어 강세, 입력 토큰 한도 **512**로 Titan보다 제약 큼 (ap-northeast-2 가용성 콘솔 확인 권장) |
| Cohere Embed v4 | ap-northeast-2 포함 가용 확인, 다국어·도메인 텍스트 성능 향상 |
...
| 가격 (참고) | Titan V2 $0.00002 / 1K tokens, Cohere Embed $0.0001 / 1K tokens ([공식 Bedrock 요금 페이지](https://aws.amazon.com/bedrock/pricing/) 확인) |

## 인증 및 호출 방식

Bedrock embedding은 **별도의 API key가 없다.** 표준 AWS API 표면(`bedrock-runtime` 엔드포인트)에 SigV4로 서명된 HTTPS 요청을 보내는 형태이며, 인증·인가는 모두 IAM으로 풀린다. IAM 외에 **Bedrock 콘솔의 Model Access 활성화**라는 별도 게이트가 하나 더 있다는 점이 가장 자주 걸리는 함정이다.

### 인증 모델
- **인증(authentication)**: AWS SigV4 — Access Key / Session Token / Instance Profile / Task Role 어디서 오든 동일
- **인가(authorization)**: IAM 정책의 `bedrock:InvokeModel` Action + 모델 ARN
- EC2/ECS/Lambda 인스턴스에 IAM Role을 붙이고, 그 Role의 정책이 Bedrock Action을 허용하면 별도 키 주입 없이 SDK가 자격증명을 자동으로 가져와 호출 성공

### 최소 IAM 정책 (Titan V2 호출용, ap-northeast-2 기준)
```json
{
  "Version": "2012-10-17",
  "Statement": [
...
- Cohere도 같이 쓰려면 `arn:aws:bedrock:ap-northeast-2::foundation-model/cohere.embed-multilingual-v3` 추가
- Knowledge Bases 경유 호출이면 `bedrock:Retrieve`, `bedrock:RetrieveAndGenerate` + KB 내부 호출 모델 ARN까지 함께 허용 필요
...
- `modelId` 만 바꾸면 Cohere(`cohere.embed-multilingual-v3`) 호출 가능. 단 페이로드 스키마(`texts`, `input_type`)는 모델별로 다름
- 대용량 인덱싱은 `invoke_model` 동기 호출 대신 **Bedrock Batch Inference** 로 비용·throughput 절감 가능

### 호출 전 체크리스트
1. **Model Access 활성화** — Bedrock 콘솔 → *Model access* → 사용할 모델 enable. 빠뜨리면 IAM이 맞아도 `AccessDeniedException: You don't have access to the model` 발생
2. **IAM 정책 attach** — 모델 ARN 단위로 최소권한 부여
3. **(옵션) VPC Interface Endpoint** — `com.amazonaws.ap-northeast-2.bedrock-runtime`로 퍼블릭 인터넷 우회
4. **(옵션) KMS** — Bedrock 로그·Knowledge Bases 데이터 암호화에 CMK 사용 시 호출 Role에 `kms:Decrypt` 추가
5. **(옵션) CloudTrail / CloudWatch** — `InvokeModel` 호출은 CloudTrail data event로 기록 가능
...
AWS 공식 레퍼런스 구현. ALB + ECS Fargate 위에 LangChain RAG 앱을 띄우고, Bedrock embedding으로 벡터를 만들어 OpenSearch Serverless에 적재한다. 청킹·임베딩·검색·생성을 분리하는 표준 패턴.

### aws-samples / rag-with-amazon-bedrock-and-pgvector
동일 RAG 워크로드를 Aurora PostgreSQL + pgvector로 구현한 샘플. 수백만 벡터 이하, 관계형 조인이 무거운 경우의 비용 효율 선택지.

### Bedrock Knowledge Bases — S3 → OpenSearch Serverless 매니지드 RAG
S3 버킷에 문서를 올리면 Bedrock이 청킹·임베딩·인덱싱을 자동 처리하고 `RetrieveAndGenerate` 한 번의 API 호출로 답변을 받는 구성. 인프라 코드를 최소화하면서 PoC를 빠르게 띄우는 패턴.

## 활용 시나리오

### 시나리오 1: 사내 문서 시맨틱 검색 (S3 + Knowledge Bases + OpenSearch Serverless)
...
### 시나리오 2: 다국어 고객 지원 RAG (Cohere Embed Multilingual + Aurora pgvector)
...
### 시나리오 3: 이미지 + 텍스트 통합 카탈로그 검색 (Titan Multimodal Embeddings G1)
이커머스에서 텍스트 쿼리로 이미지를 검색하거나, 이미지를 업로드해 유사 상품을 찾는 use case. Titan MM G1로 텍스트·이미지를 동일 공간에 임베딩하고 OpenSearch에서 k-NN 검색.

## 장단점 및 트레이드오프

| 항목 | 내용 |
|---|---|
| 장점 | (1) AWS 네이티브 통합: IAM, VPC, CloudWatch, KMS, OpenSearch/Aurora가 한 계정에서 연결 (2) Titan V2 $0.00002/1K tokens으로 가장 저렴한 축 (3) Knowledge Bases로 청킹·임베딩·인덱싱을 매니지드 처리 (4) 출력 차원 선택(256/512/1024)으로 비용/정확도 트레이드오프 가능 |
| 단점 | (1) embedding API는 `Converse` 미지원, `InvokeModel`만 사용 — 모델별 페이로드 포맷 분기 필요 (2) embedding API 레이턴시가 Cohere/Vertex/Voyage/OpenAI 대비 느린 편 (3) 리전·모델 가용성 편차, 신규 모델은 us-east-1/us-west-2에 먼저 풀림 (4) MTEB 등 일반 벤치마크에서 OpenAI 3-small과 동급, 3-large 대비 약세 |
| 트레이드오프 | AWS 락인을 받아들이는 대신 운영 자동화·컴플라이언스·통합 비용을 줄이는 선택. 임베딩 품질 자체가 절대 기준이면 OpenAI 3-large나 Voyage/Mistral-embed가 유리할 수 있고, 비용·통합·매니지드 RAG가 기준이면 Bedrock이 우위 |
...
- **embedding을 Converse API로 호출하려는 시도**: Converse는 채팅형 모델 전용. embedding은 항상 `InvokeModel`
...
- **코드에 Access Key 하드코딩**: EC2/ECS/Lambda라면 IAM Role/Instance Profile/Task Role로 자격증명을 자동 주입받는 게 표준. 키 하드코딩은 회전 불가·유출 시 폭발 반경 큼
...
- [Cohere Embed v4 on Bedrock — AWS Docs](https://docs.aws.amazon.com/bedrock/latest/userguide/model-parameters-embed-v4.html) — Cohere v4 파라미터 (High)
...
- [[aws-bedrock-agents]] — Bedrock Agents: embedding을 Knowledge Base로 소비하는 오케스트레이터
