...
promoted_from: fl-2026-06-02-aws-s3-vectors-cost-model
related:
  - "[[aws-s3-vectors-api]]"
...
> **버전 민감 항목 flag**: 아래 단가($0.06/GB·$2.5/1M·$0.004/TB·$0.002/TB)와 "최대 90% 절감" 주장은 GA 시점(2025-12) 기준 rough estimate. 최신 단가는 [Amazon S3 Pricing — Vectors](https://aws.amazon.com/s3/pricing/) 에서 확인 권장.

- **PUT**: logical GB(벡터 데이터 + 키 + 메타데이터) 단위, **약 $0.20/GB** (GA 시점). **PUT 1건당 최소 128 KB 과금** — 작은 PUT을 자주 보내면 손해. **batch가 비용에 직접 영향**
...
- **공식 절감 주장**: 전용 벡터 DB 대비 **최대 90% 비용 절감** (AWS 공식)
...
flowchart LR
  ...
  V --> PUT_C[PUT: 4 GB × $0.20 ≈ $0.80 one-shot]
...
- [Amazon S3 Pricing — Vectors section](https://aws.amazon.com/s3/pricing/) — 공식 단가표
- [Amazon S3 Vectors product page](https://aws.amazon.com/s3/features/vectors/) — 가격 모델 개요
- [Amazon S3 Vectors GA — AWS News Blog](https://aws.amazon.com/blogs/aws/amazon-s3-vectors-now-generally-available-with-increased-scale-and-performance/) — PUT 128 KB 최소·90% 절감 주장
- [AWS S3 Vectors Pricing Deep Dive vs Pinecone — Murray Cole](https://murraycole.com/posts/aws-s3-vectors-pricing-deep-dive) — 워크로드별 비용 시뮬레이션
- [How is pricing structured for AWS S3 Vector? — Milvus AI Reference](https://milvus.io/ai-quick-reference/how-is-pricing-structured-for-aws-s3-vector-features-and-operations) — 비용 차원 요약
- [Architecting GenAI at Scale — Caylent](https://caylent.com/blog/architecting-gen-ai-at-scale-lessons-from-aws-s-3-vector-store-and-the-nuances-of-hybrid-vector-storage) — hybrid 비용 패턴

## 관련 노트

- [[aws-s3-vectors-api]] — PUT/Query API의 동작 메커니즘 (비용 축의 직접 원인)
- [[bedrock-kb-s3-vectors-integration]] — KB가 내부적으로 호출하는 비용 동작 (ingestion job·Retrieve)
- [[aws-s3-vectors-cfn-privatelink-tagging]] — tag-on-create가 Cost Allocation Report에 직결, 본 비용 모델의 가시화 수단
