...
domain: ai-ml-llm
moc: "[[moc-aws-bedrock]]"
secondary_moc: "[[moc-llm-security]]"
...
promoted_from: fl-2026-06-02-aws-bedrock-guardrails
related:
...
| Denied Topics | 자연어로 주제 1줄 정의 + 예시 발화, 최대 30개 등록 (최신 한도는 AWS Docs 확인) |
| Sensitive Information | 빌트인 PII(SSN, 이름, 이메일, 주소 등) + 사용자 정규식. 동작: BLOCK 또는 ANONYMIZE(마스킹) |
| Word Filter | 정확 문자열 일치 차단(브랜드·경쟁사·욕설) |
| Contextual Grounding | RAG 응답이 source 문서로 입증 가능한지 점수화. relevance·grounding 두 임계값 설정 |
| Automated Reasoning Check | 정책 문서 기반 형식 논리 검증, 수학적 증명으로 위반 감지 (2024 re:Invent 공개, 최신 GA 여부 공식 문서 확인) |
| ApplyGuardrail API | FM 호출 분리. Bedrock 외부 모델·자체 RAG 응답·post-hoc 검증에 사용 |
| Image Content Filter | 이미지 입력의 유해 콘텐츠 분류 |

## 과금

> **버전 민감 항목 flag**: 아래 단가는 참고용이며 릴리스 주기에 따라 변경됩니다. 최신 단가는 반드시 [Amazon Bedrock pricing](https://aws.amazon.com/bedrock/pricing/)에서 확인하세요.

Guardrails는 **정책별 별도 과금**이다. 1 text unit = 최대 1,000자(영문 기준). 입력이 1,000자를 초과하면 ceil로 분할 계산(예: 5,600자 → 6 unit).

| 정책 | 단가 (참고) |
|---|---|
| Content Filter (텍스트) | text unit당 과금 (공식 페이지 확인) |
| Denied Topics | text unit당 과금 (공식 페이지 확인) |
| Sensitive Information Filter (PII) | **무료** (별도 charge 없음) |
| Word Filter | **무료** (별도 charge 없음) |
| Contextual Grounding Check | 별도 (AWS 공식 페이지에서 확인) |
| Automated Reasoning Check | 별도 (정책 문서 처리 + 검증 단가) |
| Image Content Filter | 이미지 단위 별도 단가 |
| ApplyGuardrail API (외부 모델 적용) | 위와 동일 정책 단가 적용 |

> **누적 주의**: Input·Output 양방향에 같은 정책이 켜져 있고 응답이 길면 한 요청에 동일 정책이 2회 + 응답 길이만큼 unit이 쌓인다. 대화형 챗봇에서는 모델 토큰 비용 외에 Guardrails 비용이 추가로 누적될 수 있다. PII filter는 무료이므로 켜둬도 비용 부담 없음.
...
### 금융권 챗봇 — 투자 조언 차단
은행 챗봇에서 Denied Topics에 "투자 조언", "암호화폐 추천" 등을 자연어 정의만으로 등록해 컴플라이언스 요건을 충족한 사례가 AWS 공식 블로그에 다수 게재됨.
...
### 시나리오 1: RAG 환각 방지
Bedrock Knowledge Base + Contextual Grounding을 결합. relevance·grounding 임계값을 도메인에 맞게 설정해 source 문서로 입증 불가능한 응답을 자동 차단. 법무·의료 도메인의 RAG에 필수.
...
| 단점 | (1) 영어 외 언어(특히 한국어) 정확도 일부 낮음 (2) 모든 요청이 입력+출력 양방향 평가되어 수십~수백ms 지연 추가 (3) 정책당 별도 추론 비용 발생 |
...
- [Amazon Bedrock pricing](https://aws.amazon.com/bedrock/pricing/) — Guardrails 포함 최신 단가

## 관련 노트

- [[aws-bedrock-overview]] — Guardrails가 속한 Bedrock 플랫폼 전체 개요
- [[bedrock-knowledge-bases]] — Contextual Grounding이 직접 연결되는 managed RAG 서비스
- [[aws-bedrock-agents]] — Guardrails를 attach하는 에이전트 오케스트레이션 레이어
- [[bedrock-claude-generation-models]] — Guardrails가 감싸는 generation 모델 선택 기준
- [[llama-prompt-guard-2]] — self-hosted 노선의 input guardrail classifier, ADR 9 비교 대상
- [[moc-aws-bedrock]] — 본 노트의 primary MOC (AWS Bedrock 플랫폼 관점)
- [[moc-llm-security]] — secondary MOC (LLM 가드레일·prompt injection 방어 관점)
