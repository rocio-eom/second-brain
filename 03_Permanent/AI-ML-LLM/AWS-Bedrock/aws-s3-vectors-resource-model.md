...
domain: ai-ml-llm
moc: "[[moc-aws-bedrock]]"
tags: [aws, aws-s3-vectors, vector-bucket, vector-index, resource-model, iam, s3vectors]
aliases: [S3 Vectors Resource Model, S3 Vector Bucket, S3 Vector Index, S3 Vectors Namespace]
promoted_from: fl-2026-06-02-aws-s3-vectors-vector-bucket-and-index-resource-model
related:
  - "[[aws-s3-vectors-api]]"
...
> **버전 민감 항목 flag**: 인덱스 한도(10K/bucket) 등 서비스 한도 수치는 GA 기준입니다. 최신 값은 [S3 Vectors Limitations](https://docs.aws.amazon.com/AmazonS3/latest/userguide/s3-vectors-limitations.html)에서 확인하세요.
...
graph TD
  ...
  IAM[IAM Identity Policy<br/>s3vectors:* actions] -.-> VB
  RBP[Vector Bucket Policy<br/>resource-based] -.cross-account.-> VB
...
| Block Public Access | **항상 활성, 비활성 불가** — 의도적 공개 노출 원천 차단 |
| CloudFormation 지원 | `AWS::S3Vectors::VectorBucket`, `VectorIndex`, `VectorBucketPolicy` |
...
Bedrock KB에서 vector store로 S3 Vectors를 선택하면 KB가 vector bucket과 index를 **자동 생성**한다. KB 트러블슈팅에서 가장 흔한 이슈가 KB 서비스 롤이 `s3vectors:*` 액션 권한이 없어 `AccessDeniedException`이 나는 것 — 네임스페이스 분리의 직접적 결과.
...
| 트레이드오프 | "기존 S3 운영 자산 재활용" vs "벡터 워크로드 격리·안전성". AWS는 후자를 선택. 운영팀은 새 네임스페이스에 맞춘 IAM·CFN 표준을 별도로 구성해야 한다. |
...
- [Identity and Access management in S3 Vectors — AWS Docs](https://docs.aws.amazon.com/AmazonS3/latest/userguide/s3-vectors-access-management.html) — s3vectors 네임스페이스 IAM (High)
- [S3 Vectors identity-based policy examples — AWS Docs](https://docs.aws.amazon.com/AmazonS3/latest/userguide/s3-vectors-iam-policies.html) — IAM 정책 예시 (High)
...
- [[aws-s3-vectors-api]] — 이 리소스 모델 위에서 동작하는 데이터 플레인 5개 API
- [[aws-s3-vectors-capacity-planning]] — bucket→index 계층을 활용하는 용량·샤딩 전략
- [[aws-s3-vectors-encryption]] — 인덱스별 KMS CMK override: 리소스 모델의 암호화 확장
- [[aws-s3-vectors-bucket-governance]] — bucket 생성 규칙·`s3vectors:*` IAM action·resource-based policy·BPA 강제 운영
- [[aws-s3-vectors-cost-model]] — bucket/index 단위 비용 구조
- [[aws-s3-vectors-cfn-privatelink-tagging]] — IaC + PrivateLink + 태깅으로 엔터프라이즈화
- [[bedrock-kb-s3-vectors-integration]] — KB가 이 리소스를 자동 프로비저닝하는 패턴
