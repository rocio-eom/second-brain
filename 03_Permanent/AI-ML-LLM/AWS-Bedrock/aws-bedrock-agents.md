...
promoted_from: fl-2026-06-02-aws-bedrock-agents
related: []
---
...
| Memory 기능 | 멀티턴 대화 컨텍스트를 세션 단위로 유지 |
...
| Multi-agent collaboration | Supervisor Agent가 하위 Specialist Agent를 위임 호출 |

## 과금

Bedrock Agents 자체에는 **별도 orchestration fee가 없다**. 그러나 reasoning loop가 FM을 다회 호출하므로 **단일 prompt 대비 비용 곱셈 효과**가 크다.

| 구성 요소 | 과금 |
|---|---|
| Agent orchestration | **별도 fee 없음** |
| Underlying FM | reasoning loop 1 step당 1회 호출 → 토큰 단가 × N step 누적 |
| Action Group (Lambda) | Lambda x86 기준: **$0.20 / 1M requests + $0.0000166667 / GB-sec** (ap-northeast-2 단가는 [공식 Lambda 요금 페이지](https://aws.amazon.com/lambda/pricing/) 확인) |
| Knowledge Base 호출 | embedding query 토큰 + 벡터 스토어 단가 |
| Guardrails attach | 정책별 text unit 단가 (Input·Output 양방향) |
| Memory 보관 | 별도 fee 없음 (세션 storage). 단 다음 호출에서 history가 input 토큰으로 재청구됨 |
| Code Interpreter | sandbox 실행 비용(매니지드) |
| Multi-agent collaboration | Supervisor + Specialist 각자의 FM 호출 토큰 누적 |

> **비용 추정 공식**: `Agent 1회 호출 ≈ Σ(step별 input + output 토큰 × FM 단가) + Σ(Lambda 호출 × 단가) + Σ(KB 호출 비용) + Σ(Guardrails text unit × $0.15/1K)`. PoC 전 단일 prompt 대비 5x 안전 마진으로 예산 산정 권장. Multi-agent 사용 시 10x 이상 보고 사례 존재.
