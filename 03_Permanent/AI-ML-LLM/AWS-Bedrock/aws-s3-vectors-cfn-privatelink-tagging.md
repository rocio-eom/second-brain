...
domain: ai-ml-llm
moc: "[[moc-aws-bedrock]]"
tags: [aws, aws-s3-vectors, cloudformation, privatelink, vpc-endpoint, tagging, iac, enterprise-readiness]
aliases: [S3 Vectors CloudFormation, S3 Vectors PrivateLink, S3 Vectors VPC endpoint, S3 Vectors tagging, S3 Vectors Enterprise Operations]
promoted_from: fl-2026-06-02-aws-s3-vectors-cloudformation-privatelink-tagging
related:
  - "[[aws-s3-vectors-api]]"
...
- **CloudFormation 리소스 타입**: `AWS::S3Vectors::VectorBucket`, `AWS::S3Vectors::Index`, `AWS::S3Vectors::VectorBucketPolicy`
- **PrivateLink (Interface VPC Endpoint)**: 인터넷 게이트웨이/NAT 없이 VPC에서 S3 Vectors API에 사설 접근. 서비스 이름 `com.amazonaws.<region>.s3vectors`, private DNS 활성화 시 `s3vectors.<region>.api.aws`로 라우팅. **엔드포인트 정책**으로 vector bucket/index 단위 접근 제어
...
| CloudFormation | `AWS::S3Vectors::Index` | dim/metric/non-filt key를 코드로 고정 (불변성 가시화) |
...
`AWS::S3Vectors::Index`를 모듈/CDK construct로 캡슐화. **차원·메트릭·non-filterable key를 코드 리뷰**에서 점검(이들은 불변이므로 변경 = blue/green 재인덱싱). PR 단위 거버넌스.
...
- [VPC endpoints for S3 Vectors — AWS Docs](https://docs.aws.amazon.com/AmazonS3/latest/userguide/s3-vectors-privatelink.html) — PrivateLink 구성
- [Amazon S3 Vectors now GA — AWS News Blog](https://aws.amazon.com/blogs/aws/amazon-s3-vectors-now-generally-available-with-increased-scale-and-performance/) — CFN/PrivateLink/Tagging GA 발표
- [AWS::S3Vectors::VectorBucket — CloudFormation](https://docs.aws.amazon.com/AWSCloudFormation/latest/TemplateReference/aws-resource-s3vectors-vectorbucket.html) — VectorBucket 공식
- [AWS::S3Vectors::Index — CloudFormation](https://docs.aws.amazon.com/AWSCloudFormation/latest/TemplateReference/aws-resource-s3vectors-index.html) — Index 공식 (※ `VectorIndex`가 아닌 `Index` — 명명 주의)
- [AWS::S3Vectors::VectorBucketPolicy — CloudFormation](https://docs.aws.amazon.com/AWSCloudFormation/latest/TemplateReference/aws-resource-s3vectors-vectorbucketpolicy.html) — VectorBucketPolicy 공식
- [AWS::EC2::VPCEndpoint — CloudFormation](https://docs.aws.amazon.com/AWSCloudFormation/latest/TemplateReference/aws-resource-ec2-vpcendpoint.html) — VPC endpoint 정의
- [Working with S3 Vectors — AWS Docs](https://docs.aws.amazon.com/AmazonS3/latest/userguide/s3-vectors.html) — 운영 일반
- [Amazon Bedrock AgentCore VPC/PrivateLink/CFN/Tagging — AWS What's New](https://aws.amazon.com/about-aws/whats-new/2025/09/amazon-bedrock-agentcore-runtime-browser-code-interpreter-vpc-privatelink-cloudformation-tagging/) — 동시 트랙 사례

## 관련 노트

- [[aws-s3-vectors-api]] — S3 Vectors 데이터 플레인 5개 API. 본 노트(운영·거버넌스 plane)와 대비되는 application plane
- [[bedrock-kb-s3-vectors-integration]] — KB 통합 패턴. 본 노트의 운영 기반 위에서 동작하는 매니지드 RAG 소비자
- [[aws-bedrock-overview]] — Bedrock 플랫폼 컨텍스트
