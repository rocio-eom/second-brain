...
tags: [vector-search, operations, reindex, monitoring, embedding-versioning, retrieval-eval]
aliases: [Vector Search Operations, Vector Search Reindex Strategy, RAG Index Operations]
promoted_from: [fl-2026-06-02-vector-search-operations]
related:
...
Vector 검색·색인 시스템을 **운영** 단계에서 다루는 핵심 활동: ① 임베딩 모델/차원/metric 변경 시 **재인덱싱(reindex)** ② 인덱스 수명/버전 관리 ③ 검색 품질·latency·비용의 **지속 모니터링** ④ 데이터 drift·정책 변경 대응. 설계 단계 결정은 [[vector-search-architecture]] 참조.
...
자매 노트: 설계 결정은 [[vector-search-architecture]], 제품 선택은 [[vector-database-comparison]], OpenSearch 특화는 [[opensearch-knn-vector-search]].
...
- [[vector-search-architecture]] — 운영 단계가 전제로 하는 설계 결정
- [[vector-database-comparison]] — 제품별 운영 모델·shadow index 지원
- [[vector-search-performance-tuning]] — 운영 중 SLO 회귀 대응의 튜닝 방법론
- [[opensearch-knn-vector-search]] — OpenSearch ILM·alias·hot-warm 운영 패턴
- [[recall-vs-filter-tradeoff]] — 운영 모니터링에서 recall 저하 원인 식별

## 출처 fleeting

- `fl-2026-06-02-vector-search-operations.md`
