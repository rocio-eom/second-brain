...
tags: [vector-search, performance-tuning, ann, hnsw, ivf, quantization, optimization]
aliases: [Vector Search Performance Tuning, ANN Tuning Overview, Vector Search Optimization, Vector DB Performance Tuning]
promoted_from: [fl-2026-06-02-vector-search-performance-tuning-overview]
related:
...
3. **Layer 1 — 인덱스**: HNSW면 `ef_search`(쿼리 시 변경 가능)부터, IVF면 `nprobe`부터. M·efConstruction·nlist는 빌드 재실행 비용이 있어 후순위 — 상세는 [[ann-index-parameters]]
4. **Layer 2 — 양자화**: 메모리/비용이 1순위 SLO면 우선. SQ → PQ → Binary 순으로 추가 압축 시도, 각 단계에서 recall 손실 측정
5. **Layer 3 — 시스템/쿼리**: 필터링은 selectivity로 pre/post 결정. hybrid rerank는 top-N (보통 50~200)로 후처리. throughput은 replica + batched query — 상세는 [[vector-search-system-query-tuning]]
...
- [[ann-index-parameters]] — Layer 1: HNSW · IVF 파라미터 상세
- [[vector-search-system-query-tuning]] — Layer 3: filter · hybrid · rerank · batching · replica
- [[vector-search-operations]] — 튜닝 결과의 운영 반영 (shadow index · eval harness)
- [[vector-database-comparison]] — 제품별 튜닝 노브 노출 차이
- [[recall-vs-filter-tradeoff]] — Layer 3 필터링이 recall에 미치는 보편 한계

## 출처 fleeting

- `fl-2026-06-02-vector-search-performance-tuning-overview.md`
