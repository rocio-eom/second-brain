...
tags: [opensearch, knn, vector-search, hnsw, faiss, lucene, hybrid-search, rrf, neural-search, rag]
aliases: [OpenSearch k-NN, OpenSearch Vector Search, k-NN Plugin, OpenSearch Neural Search, OpenSearch Hybrid Search]
promoted_from: fl-2026-06-02-vector-search-opensearch-knn
related:
...
OpenSearch에 내장된 **k-NN 플러그인**으로 BM25 lexical 검색과 dense vector 검색을 한 클러스터에서 동시에 운영하는 아키텍처. v3.0(2025-05)에서 vector engine 성능이 이전 대비 최대 **9.5배** 향상되며 본격적인 vector DB 대안으로 자리 잡았다. RAG·시맨틱 검색에 별도 vector DB 도입 없이 기존 OpenSearch 클러스터를 그대로 활용할 수 있다는 점이 큰 이점.
...
> **버전 민감 항목**: 본 노트의 버전 의존 사실 — OpenSearch 3.0 9.5배 향상(2025-05 release), Nmslib 2.19 deprecation, Neural Search RRF 2.19+ — 은 2026-06 기준. 최신 사실은 OpenSearch release notes 확인.
...
- **안티패턴 1: 임베딩 모델 변경 시 부분 reindex** → 모델이 다른 vector를 한 인덱스에 섞으면 거리 계산이 무의미 → 새 인덱스 생성 후 alias swap. 의미공간 비호환 자체는 [[embedding-model-lock-in]] 참조.
...
- **안티패턴 4: pre-filter 남용** → 큰 필터 셋에 pre-filter를 적용하면 ANN 효율 저하 → efficient filter 모드(2.x) 활용. 필터 모드별 트레이드오프는 [[recall-vs-filter-tradeoff]] 참조.
...
- [[vector-store-decision-matrix]] — RAG용 벡터 저장소 선택 4축 매트릭스에서 OpenSearch의 위치
- [[recall-vs-filter-tradeoff]] — ANN+필터 보편 한계: pre/post/efficient filter 모드 비교의 일반론
- [[embedding-model-lock-in]] — 모델 교체 시 의미공간 비호환과 alias swap 필요성의 일반화
- [[bedrock-knowledge-bases]] — Bedrock Knowledge Bases가 OpenSearch Serverless를 백엔드로 사용할 때의 RAG 통합 관점
