...
tags: [vector-search, hnsw, ivf, ann, index-parameters, performance-tuning]
aliases: [HNSW Tuning, IVF Tuning, ANN Index Parameters, Vector Index Hyperparameters]
promoted_from: [fl-2026-06-02-vector-search-performance-tuning-index-params]
related:
...
- [[vector-search-performance-tuning]] — 4-layer 튜닝의 Layer 1 (본 노트가 상세화한 영역)
- [[vector-search-system-query-tuning]] — Layer 3에서 ef_search·nprobe와 결합되는 필터·hybrid
- [[vector-search-operations]] — `M`·`nlist` 변경 시 발생하는 재빌드의 운영 패턴 (shadow + alias swap)
- [[recall-vs-filter-tradeoff]] — `ef_search` 상향으로 회복하기 어려운 필터 기반 recall 저하

## 출처 fleeting

- `fl-2026-06-02-vector-search-performance-tuning-index-params.md`
