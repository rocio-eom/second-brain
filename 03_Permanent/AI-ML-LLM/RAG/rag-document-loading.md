...
domain: ai-ml-llm
moc: "[[moc-rag]]"
tags: [rag, ingestion, document-loading, document-parsing, pdf-parsing, unstructured, llamaparse, docling, layout-aware]
aliases: [Document Loading, Document Parsing, RAG Document Loading, RAG Document Parsing, Document Loaders, Unstructured Document Parsing, Layout-Aware Parsing]
promoted_from: fl-2026-06-02-rag-ingestion-document-loading
related:
...
Document Loading & Parsing은 RAG ingestion 파이프라인의 최상단 단계다. 다양한 포맷(PDF/HTML/DOCX/PPTX/MD/이미지)으로 존재하는 원본 문서를 LLM이 처리 가능한 **구조화된 텍스트 + 메타데이터**로 변환한다. 단순히 텍스트를 추출하는 작업이 아니라, **레이아웃·표·헤딩 계층·읽기 순서를 보존**해야 downstream chunking과 retrieval 품질이 결정된다.
...
> **버전 민감 항목**: 개별 도구의 한국어 OCR 정확도·비용·포맷 지원 범위는 릴리스에 따라 변동. 최신값은 각 도구 공식 문서 확인.

## 시스템 아키텍처

Document Loader는 **Source Adapter → Format Detector → Format-specific Parser → Element Normalizer → Output Schema** 5단계로 구성된다. Layout-aware 도구는 Parser 단에 vision/OCR 모델이 추가된다.
...
### IBM (Docling / watsonx)
IBM은 내부 RAG 스택에 자사 오픈소스 Docling을 사용한다. DocLayNet 데이터셋으로 학습된 레이아웃 모델로 표·수식·이미지를 구조 보존 추출 후 Granite 모델로 임베딩하는 파이프라인을 GitHub에 공개했다.

### Notion AI
Notion은 블록 기반 자체 스키마를 RAG ingestion에 활용한다. 외부 문서 import 시 레이아웃 파싱을 거쳐 Notion 블록 트리로 매핑, retrieval 시 블록 계층을 그대로 활용해 컨텍스트 윈도우 효율을 높인다.
...
| 단점 | Layout 모델 사용 시 처리량 급락(text-only 대비 수십 배 느림), GPU 비용, 한국어 OCR 정확도 격차 |
...
- **안티패턴 5**: 한글 PDF에 영문 OCR 모델 적용 → 인식률 급락 → **대안**: 언어 감지 후 multilingual OCR(Tesseract `kor+eng`, AWS Textract 한글) 라우팅

## 참고 자료

- [Unstructured.io Docs](https://docs.unstructured.io/) — 25+ 포맷 partition API 공식 문서 (High)
- [LlamaParse Documentation](https://docs.cloud.llamaindex.ai/llamaparse/getting_started) — LLM 기반 PDF 파서 가이드 (High)
- [Docling GitHub (IBM)](https://github.com/DS4SD/docling) — 오픈소스 layout-aware 파서 (High)
- [Anthropic - Introducing Contextual Retrieval](https://www.anthropic.com/news/contextual-retrieval) — ingestion 품질이 retrieval에 미치는 영향 사례 (High)
- [Apache Tika](https://tika.apache.org/) — JVM 기반 범용 콘텐츠 추출 (High)
- [PyMuPDF Docs](https://pymupdf.readthedocs.io/) — 고속 PDF 파서 레퍼런스 (High)
- [LangChain Document Loaders](https://python.langchain.com/docs/integrations/document_loaders/) — 200+ 소스 어댑터 카탈로그 (High)
- [Marker GitHub](https://github.com/VikParuchuri/marker) — PDF→Markdown 변환 도구 (Mid)

## 관련 노트

- [[chunking-strategy-methodology]] — 파싱 완료 후 이어지는 청킹 설계·품질 사이클
- [[chunking-failure-modes]] — 파싱 품질 저하가 downstream에서 실패로 나타나는 패턴
- [[confluence-storage-format-parser]] — Confluence HTML(XHTML + ac:*/ri:* namespace) 파싱 전용 도구 선택
- [[confluence-doc-type-classification-heuristic]] — Confluence 페이지 타입 분류 (파싱 후 라우팅 전 단계)
- [[moc-rag]] — 본 노트 소속 MOC
