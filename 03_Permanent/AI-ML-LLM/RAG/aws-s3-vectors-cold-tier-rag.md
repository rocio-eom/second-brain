...
aliases: [Cold-tier RAG, Archive Semantic Search, S3 Vectors Archive Pattern, S3 Vectors Cold-only RAG]
promoted_from: fl-2026-06-02-aws-s3-vectors-cold-tier-rag-archive
related:
  - "[[aws-s3-vectors-api]]"
...
# Cold-tier RAG Archive (S3 Vectors)
...
> **버전 민감 항목 flag**: ~$0.06/GB 단가는 ap-northeast-2 기준 GA 시점 값입니다. 최신 가격은 [S3 Vectors pricing](https://aws.amazon.com/s3/pricing/)에서 확인하세요.
...
### 사내 운영 매뉴얼·런북
과거 운영 사고·복구 절차 텍스트. 신규 인시던트 시 검색 → 패턴 매칭. 일~월간 검색 회수 한 자리 수. OSS Serverless ~$350/mo가 사실상 100% 낭비되는 시나리오.
...
| 트레이드오프 | "검색 빈도 ↔ 비용". 검색이 드물수록 cold-only가 압도적, 빈도가 늘면 hot/cold tiering hybrid로 전환. **PoC 단계 cold-only → 운영 중 빈도 측정 → 필요 시 hybrid 승격**이 자연스러운 진화. |
...
- [[aws-s3-vectors-api]] — PutVectors·QueryVectors 데이터 플레인: cold-tier에서 적용되는 배치 한도·처리량
- [[aws-s3-vectors-cost-model]] — idle 비용 구조: cold-tier 경제성의 기반
- [[aws-s3-vectors-capacity-planning]] — 인덱스 분할 전략: 아카이브 도메인별 인덱스 설계
- [[aws-s3-vectors-encryption]] — KMS CMK 패턴: 컴플라이언스 아카이브에 적용되는 암호화 격리
- [[bedrock-kb-s3-vectors-integration]] — KB Retrieve 통합: cold-tier 단순화의 핵심 레이어
- [[vector-store-decision-matrix]] — cold-tier 선택 의사결정 프레임워크
