...
domain: ai-ml-llm
moc: "[[moc-rag]]"
tags: [rag, ingestion, production, ragops, dlq, observability, monitoring, cost, langfuse, helicone]
aliases: [RAG Ingestion Production Operations, RAGOps Ingestion, Embedding Ingestion Pipeline Observability, RAG Ingestion DLQ, RAG Ingestion Cost Attribution]
promoted_from: fl-2026-06-02-embedding-ingestion-pipeline-production-ops
related:
...
# RAG Ingestion Production Operations

## 핵심 요약

RAG ingestion 파이프라인이 production에 들어갔을 때 발생하는 **운영 3대 문제 — 실패 격리(DLQ) · 관측(observability) · 비용 통제**에 집중한다. 파이프라인 자체의 컴포넌트 구성·throughput 튜닝·incremental 전략은 [[rag-document-loading]] / [[rag-embedding-generation]] / [[embedding-incremental-cdc-ingestion]]를 참조하고, 본 노트는 그 위에 얹는 **운영 평면(operational plane)**을 다룬다.
...
> **버전 민감 항목**: 아래 "유사 기술 비교" 표의 도구별 라이선스·기능·통합 방식은 2026-06 기준. 최신값은 각 도구 공식 docs 확인.
...
- [[rag-document-loading]] — RAG ingestion 파이프라인 최상단의 문서 로딩·파싱
- [[rag-embedding-generation]] — 대규모 배치 임베딩 throughput·지연·비용 튜닝
- [[embedding-incremental-cdc-ingestion]] — incremental CDC 기반 변경 chunk만 재임베딩하는 운영 패턴
- [[embedding-model-lock-in]] — embed_model 교체 시 의미공간 비호환과 schema registry 필요성
- [[chunking-failure-modes]] — chunking 단계 실패 모드 (anti-pattern과 cross-link)
