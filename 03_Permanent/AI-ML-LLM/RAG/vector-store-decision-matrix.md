...
tags: [aws, vector-db, decision-matrix, rag, opensearch, pinecone, pgvector, s3-vectors, memorydb]
aliases: [Vector Store Decision Matrix, AWS Vector DB Selection, Vector Store Comparison, RAG Vector DB Decision]
promoted_from: fl-2026-06-02-aws-s3-vectors-vector-store-decision-matrix
related:
  - "[[aws-s3-vectors-api]]"
...
AWS 생태계에서 RAG 워크로드의 벡터 저장소는 **S3 Vectors / OpenSearch Service(+Serverless) / Aurora PostgreSQL pgvector / MemoryDB / Pinecone / Bedrock KB** 등으로 다양하다. 결정 축은 **(1) latency SLA, (2) 비용/idle, (3) 워크로드 빈도, (4) 검색 풍부도, (5) 기존 RDB/검색엔진과의 통합, (6) 운영 부담**. **단일 정답 없음 — 워크로드 빈도 분포·SLA·기존 자산 기준으로 결정**.

- **Latency 영역**: sub-ms(MemoryDB) / sub-10ms(OSS) / ~100ms(S3V warm) / sub-second(S3V cold).
- **Idle 비용 영역**: ~0(S3V) / 인스턴스 시간(pgvector·MemoryDB) / OCU 시간(OSS Serverless) / 월 최소(Pinecone).
- **AWS Prescriptive Guidance**: 공식 결정 가이드 존재. 의사결정 트리 기반.
- **하이브리드도 옵션**: cold=S3V + hot=OSS 조합이 표준 패턴. 단일로 시작 → 측정 → hybrid 전환.
...
> **버전 민감 항목 flag**: 아래 Idle 비용 수치(OSS Serverless ~$350+/mo, Pinecone $50+/mo 등)는 참고용 rough estimate입니다. 최신 단가는 [Amazon OpenSearch pricing](https://aws.amazon.com/opensearch-service/pricing/) 및 [Pinecone pricing](https://www.pinecone.io/pricing/) 에서 확인하세요.
...
| **OpenSearch Serverless** | ~$350+/mo (OCU 최소) | sub-10ms | 매니지드 + hybrid | OCU 최소 비용 | hot·매니지드 |
...
S3 Vectors가 OSS 대비 비용 절감 효과 외에, 워크로드 분류 시 OSS도 여전히 필요한 영역을 명확히 분리. Hybrid 결정 가이드.
...
- [[aws-s3-vectors-api]] — S3 Vectors 데이터 플레인 API (PutVectors / QueryVectors 등)
- [[bedrock-kb-s3-vectors-integration]] — KB + S3V 통합 패턴: cold·archive RAG의 매니지드 구현
- [[bedrock-knowledge-bases]] — KB가 지원하는 vector store 옵션 전체 개요
- [[hot-cold-data-tiering]] — S3V(Cold)·OSS(Hot) hybrid 패턴의 이론적 기반
- [[moc-vector-search]] — 본 노트 소속 MOC
