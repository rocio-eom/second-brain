...
domain: ai-ml-llm
moc: "[[moc-rag]]"
tags: [rag, embedding, korean-nlp, korean-embedding, code-switching, vector-search, retrieval]
aliases: [Korean RAG Embedding Selection, Korean-First Embedding Models, Korean-English Code-Switching Retrieval, Korean embedding model selection]
promoted_from: fl-2026-06-02-korean-rag-embedding-selection
related:
...
- **한국어 토큰 효율**: 영어 중심 토크나이저는 한국어 문장을 영어 대비 훨씬 더 많은 토큰으로 분해 → 동일 context 길이 대비 실질 활용도 감소. 한국어-영어 코드스위칭 임베딩 평가(HiKE, ConCSE) 연구에서 이 비대칭이 retrieval 성능에 직접 영향을 줌을 확인
- **자연어 내 영어 혼재 케이스**: 한국어 본문 + 영어 기술 용어/고유명사(예: "Kafka의 Streams API를 이용해…")가 빈번 → 영어 토큰 임베딩 품질도 동시에 보장되어야 함 → **단일 한국어 전용 모델보다 한국어 강화 다국어 모델이 안전**
- **벤치마크 단독 판단 금지**: MTEB-ko-retrieval 리더보드 + 자체 도메인 데이터 평가 + 태스크별(검색/리랭킹/QA) 분해, "3중 검증"이 사실상 표준

## 시스템 아키텍처

한국어 RAG의 임베딩 레이어는 단순 "모델 1개"가 아니라 **passage encoder + query encoder + 리랭커**의 3단 구조로 분리된다. 한국어/영어 혼재를 견디려면 토크나이저 단계에서 정규화를 한 번 거치는 것이 안정적이다.
...
> **버전 민감 항목 flag**: 아래 모델 사양(차원·컨텍스트·성능 순위)은 신규 모델 출시로 변화할 수 있습니다. 최신 순위는 [MTEB Leaderboard — Korean retrieval 필터](https://huggingface.co/spaces/mteb/leaderboard)로 재확인하세요.
...
| 적합 케이스 | 한국어 비중 80%+ 폐쇄망 RAG | 한/영/일/중 혼재 멀티테넌트 | Upstage Solar LLM 스택 통합 | PoC 빠른 시작, 글로벌 SaaS |

## 실제 사례

### SK하이닉스 RAG 플랫폼 (AWS 사례)

사내 기술 문서 RAG 구축 시 **Jina Embeddings v2 base (en)** 을 SageMaker Endpoint로 운영하고 Amazon OpenSearch를 벡터 저장소로 사용. 한국어 문서가 다수임에도 영어 임베딩 모델을 선택한 사례 — 사내 도메인 평가셋 기준으로 비용/지연/정확도 균형에 맞춰 선택했다는 점이 핵심. → **리더보드와 실제 도메인 결과가 갈릴 수 있음**을 보여주는 반례로 자주 인용된다.
...
**적용**: 청크 300~500 토큰 / structure-aware 청킹. dense + BM25 하이브리드(2-stage). 리랭커가 top-50 → top-5 압축.
...
| 항목 | 내용 |
|---|---|
| 장점 (한국어 특화 모델) | 동일 도메인 데이터에서 다국어 범용 모델보다 Recall@k가 높게 보고되는 사례 다수 (KURE 논문, MTEB-ko-retrieval 리더보드). 한국어 토크나이저 효율로 컨텍스트 활용도 향상. |
...
- **안티패턴 5**: 청크 크기를 임베딩 모델 max length(예: 8192)에 맞춰 키움 → 청크 내 의미 희석으로 retrieval 정확도 하락. **대안**: 200~500 토큰 의미 단위 청킹 + 부모-자식(parent-child) 청킹으로 컨텍스트 보강.
...
- [HiKE: Korean-English Code-Switching Evaluation (arXiv 2509.24613)](https://arxiv.org/pdf/2509.24613) — 한영 코드스위칭 평가 프레임워크 및 토큰 비대칭 분석
- [MTEB Leaderboard (Hugging Face)](https://huggingface.co/spaces/mteb/leaderboard) — Korean retrieval 태스크 포함 다국어 리더보드

## 관련 노트

- [[cohere-embed-multilingual-v3]] — 다국어 임베딩 옵션 중 하나. 한국어 품질 비교 대상
- [[aws-bedrock-embedding]] — AWS Bedrock 매니지드 임베딩(Titan v2·Cohere). 운영 비용 비교 대상
- [[confluence-doc-type-classification-heuristic]] — 문서 유형별 청킹 전략을 결정하는 인접 노트. 임베딩 선정 후 청킹 전략과 연동
- [[moc-embedding]] — 임베딩 모델 전반 인덱스. 본 노트의 형제 노트들
