...
domain: ai-ml-llm
moc: "[[moc-rag]]"
tags: [rag, ingestion, multi-modal, image-ingestion, table-extraction, audio-transcription, video-rag, colpali, clip, whisper]
aliases: [Multi-modal Ingestion, Multimodal RAG, Image Ingestion, Table Ingestion, Audio Ingestion, Visual RAG, Cross-modal Embedding]
promoted_from:
...
- **안티패턴 3**: 오디오 전체를 단일 chunk로 임베딩 → segment-level query 불가 → **대안**: timestamp 기반 30~60초 segment 청킹, segment URL metadata 보존. chunking 일반론은 [[chunking-strategy-methodology]] 참조.
- **안티패턴 4**: 원본 binary를 vector DB에 저장 → DB 비대화·비용 폭증 → **대안**: 원본은 object storage(S3), vector DB는 reference URL만.
- **안티패턴 5**: caption만 보존하고 원본 삭제 → 답변에 시각 자료 첨부 불가, audit 불가 → **대안**: 원본은 항상 유지 (retention 정책 별도). 원본 삭제 시 propagation은 [[rag-deletion-propagation]] 참조.
...
- `fl-2026-06-02-rag-ingestion-multi-modal-ingestion` (cluster: `rag-ingestion`, 2026-06-06 promote)

## 관련 노트

- [[rag-vector-index-lifecycle]] — sibling: modality별 인덱스 lifecycle 분리 운영
- [[rag-chunk-metadata-extraction]] — sibling: unified chunk schema 통일과 modality metadata 부착
- [[rag-preprocessing-cleaning]] — sibling: modality별 cleaning 정책 (caption·transcript 별도)
- [[rag-deletion-propagation]] — sibling: 이미지/오디오 원본의 삭제 전파 (object storage 동기)
- [[rag-document-loading]] — modality splitter 직전의 parser layer
- [[chunking-strategy-methodology]] — modality별 chunking 단위 결정의 일반론
